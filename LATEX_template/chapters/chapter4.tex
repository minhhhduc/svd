...
\text{}\indent Các thí nghiệm được thực hiện trên một hệ thống di động sử dụng bộ vi xử lý AMD Ryzen 5 6600HS (Zen 3+, mã hiệu Rembrandt) với thông số kỹ thuật như sau:
\begin{itemize}
...
\text{}\indent Để đánh giá hiệu năng của các thuật toán, sử dụng bộ dữ liệu gồm các ma trận được sinh ngẫu nhiên với kích thước và đặc điểm đa dạng. Quá trình sinh dữ liệu ngẫu nhiên với các kích thước ma trận khác nhau.

\paragraph{Đặc điểm dữ liệu:}
\begin{itemize}
    \item \textbf{Giá trị:} Các phần tử trong ma trận là số thực ngẫu nhiên (float) nằm trong khoảng $[0, 1000]$.
    \item \textbf{Định dạng lưu trữ:} Mỗi ma trận được lưu trong một file văn bản (.txt). Dòng đầu tiên chứa hai số nguyên $m$ và $n$ tương ứng với số hàng và số cột. Các dòng tiếp theo chứa giá trị của ma trận.
\end{itemize}

\paragraph{Các kích thước thử nghiệm:} Tạo ra các bộ dữ liệu với kích thước tăng dần để kiểm tra khả năng mở rộng (scalability) của thuật toán. Các kích thước cụ thể bao gồm:

\begin{table}[H]
    \centering
    \caption{Danh sách các kích thước ma trận sử dụng trong thực nghiệm}
    \label{tab:dataset_sizes}
    \begin{tabular}{|c|c|c|c|}
        \hline
        \textbf{STT} & \textbf{Kích thước ($m \times n$)} & \textbf{Số phần tử} & \textbf{Mô tả} \\ \hline
        1 & $200 \times 128$ & 25,600 & Ma trận nhỏ \\ \hline
        2 & $490 \times 64$ & 31,360 & Ma trận nhỏ, hẹp \\ \hline
        3 & $500 \times 256$ & 128,000 & Ma trận trung bình \\ \hline
        4 & $1000 \times 178$ & 178,000 & Ma trận trung bình, dài \\ \hline
        5 & $750 \times 512$ & 384,000 & Ma trận trung bình lớn \\ \hline
        6 & $3333 \times 768$ & 2,559,744 & Ma trận lớn \\ \hline
        7 & $3000 \times 1024$ & 3,072,000 & Ma trận rất lớn \\ \hline
    \end{tabular}
\end{table}
...
\subsection{Thuật toán giảm chiều dữ liệu với SVD}
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{images/compare_SVD.png}
    \caption{Biểu đồ so sánh thời gian thực thi thuật toán giảm chiều dữ liệu với SVD giữa tuần tự và song song}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Kết luận:} Thuật toán giảm chiều dữ liệu với SVD song song chỉ thực sự phát huy tác dụng với các bộ dữ liệu lớn, nơi khối lượng tính toán đủ lớn để che lấp chi phí quản lý luồng.
    \end{itemize}
\end{itemize}

\section{Tổng hợp và So sánh hiệu năng}

\text{}\indent Dựa trên các kết quả thực nghiệm chi tiết ở trên, bảng dưới đây tổng hợp các chỉ số hiệu năng quan trọng của các thuật toán đã triển khai:

\begin{table}[H]
\centering
\caption{Bảng so sánh hiệu năng các thuật toán song song}
\label{tab:performance_summary}
\resizebox{\textwidth}{!}{%
\begin{tabular}{|l|c|c|l|}
\hline
\textbf{Thuật toán} & \textbf{Speedup tối đa ($S_{max}$)} & \textbf{Điểm bão hòa (Luồng)} & \textbf{Đặc điểm chính} \\ \hline
Nhân ma trận (Cannon) & $\approx 11.3$ & 9 ($3 \times 3$) & Tăng tốc siêu tuyến tính nhờ tối ưu cache (blocking). \\ \hline
Nhân ma trận (DNS) & $\approx 8.5$ & 8 ($2 \times 2 \times 2$) & Hiệu quả với số luồng lập phương, chi phí truyền thông cao. \\ \hline
Jacobi (Trị riêng) & $\approx 4.4$ & - & Cần đồng bộ hóa liên tục, hiệu quả cao với $N \ge 1024$. \\ \hline
SVD & $\approx 4.6$ & - & Chỉ hiệu quả với dữ liệu lớn ($N > 3000$), overhead lớn ở $N$ nhỏ. \\ \hline
Sắp xếp (Argsort) & $\approx 4.4$ & 8 & Bị giới hạn bởi băng thông bộ nhớ khi merge. \\ \hline
Chuyển vị (Transpose) & $\approx 5.0$ & 6 & Memory-bound, bão hòa sớm do nghẽn băng thông. \\ \hline
Căn bậc hai (Square) & $\approx 3.8$ & 6-8 & Tác vụ quá nhẹ, chi phí quản lý luồng chiếm ưu thế. \\ \hline
\end{tabular}%
}
\end{table}

\paragraph{Nhận xét chung:}
\begin{itemize}
    \item \textbf{Nhóm thuật toán tính toán nặng (Compute-bound):} Bao gồm nhân ma trận (Cannon, DNS) và phân rã SVD/Jacobi. Các thuật toán này đạt hệ số tăng tốc tốt và có khả năng mở rộng (scalability) cao khi kích thước dữ liệu tăng. Đặc biệt, thuật toán Cannon đạt hiệu suất vượt trội nhờ khai thác tốt bộ nhớ cache.
    \item \textbf{Nhóm thuật toán giới hạn bộ nhớ (Memory-bound):} Bao gồm Chuyển vị, Sắp xếp và các phép toán trên mảng đơn giản. Các thuật toán này nhanh chóng đạt điểm bão hòa ở số lượng luồng thấp (6-8 luồng) do băng thông bộ nhớ của hệ thống không đáp ứng kịp tốc độ xử lý của CPU đa nhân.
\end{itemize}
